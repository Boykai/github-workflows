...
test.describe('Authentication', () => {
  test('should display login page when not authenticated', async ({ page }) => {
    ...
    await expect(page.locator('h1')).toContainText('Ready Set Go');
  ...
  test('should show login page without loading spinner', async ({ page }) => {
    ...
    await expect(page.locator('h1')).toContainText('Ready Set Go', { timeout: 5000 });
...
test.describe('Page Structure', () => {
  test('should have correct page title', async ({ page }) => {
    ...
    // Check page title
    await expect(page).toHaveTitle(/Ready Set Go/i);
  });
