        ...
        return {
          ...
          stages: prev.stages.map((s) => {
            ...
            return {
              ...
              execution_mode: updatedAgents.length >= 2 ? 'parallel' as const : s.execution_mode,
            };
          }),
        };
      });
  ...
  const removeAgentFromStage = useCallback((stageId: string, agentNodeId: string) => {
    setPipeline((prev) => {
      ...
      return {
        ...
        stages: prev.stages.map((s) => {
          if (s.id !== stageId) return s;
          const updatedAgents = s.agents.filter((a) => a.id !== agentNodeId);
          return {
            ...s,
            agents: updatedAgents,
            execution_mode: updatedAgents.length < 2 ? 'sequential' as const : s.execution_mode,
          };
        }),
      };
    });
    ...
    (stageId: string, mode: 'sequential' | 'parallel') => {
      setPipeline((prev) => {
        ...
        return {
          ...
          stages: prev.stages.map((s) => {
            if (s.id !== stageId) return s;
            // Only allow parallel if 2+ agents
