...
function ActiveStatusBadge({ isActive }: { isActive: boolean }) {
  return isActive ? (
    ...
    <span className={cn('inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-medium', STATUS_COLORS.neutral.bg + '/50', STATUS_COLORS.neutral.text)}>
      <span className={cn('w-1.5 h-1.5 rounded-full', STATUS_COLORS.neutral.dot + '/40')} />
      Inactive
    </span>
  );
}
