...
export function ChoreCard({ chore, projectId }: ChoreCardProps) {
  ...
  return (
    <div className="flex flex-col gap-1.5 p-3 rounded-md border border-border bg-card hover:bg-accent/50 transition-colors">
      ...
      <div className="flex items-center justify-between gap-2">
        ...
        <button
          ...
          className={cn(
            ...
            chore.status === 'active'
              ? cn(STATUS_COLORS.success.bg, STATUS_COLORS.success.text, 'hover:bg-green-200 dark:hover:bg-green-900/50')
      ...
      {nextTriggerInfo && <p className="text-xs text-muted-foreground">⏱ {nextTriggerInfo}</p>}
      ...
      <div className="flex items-center justify-between mt-1">
        <button
          ...
          title={
            chore.current_issue_number
              ? 'An open issue may exist — will verify before triggering'
              : 'Manually trigger this chore'
          }
        >
