...
- Python 3.11+ (backend), TypeScript ~5.4 (frontend) + FastAPI, Pydantic 2, httpx, aiosqlite (backend); React 18, TanStack Query, Vite (frontend) (011-signal-chat-integration)
- SQLite via aiosqlite (existing pattern — new migration for `signal_connections` and `signal_messages` tables) (011-signal-chat-integration)
