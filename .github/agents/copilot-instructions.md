...
- Python 3.13 (backend), TypeScript 5.9 (frontend), Bicep (IaC — generated by agent, not compiled at dev time) + FastAPI + Pydantic + GitHubKit + PyNaCl (backend), React 19 + Vite 7 + TanStack Query (frontend), Azure MCP + Context7 + CodeGraphContext (agent tooling) (050-architect-azure-iac)
- aiosqlite (SQLite — no schema changes; Azure credentials stored in GitHub Secrets, not local DB) (050-architect-azure-iac)
