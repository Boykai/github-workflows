...
- SQLite (WAL mode) via aiosqlite, file-backed at `/app/data/settings.db` (010-bug-bash, 012-deep-security-review — existing schema; no changes required for security hardening)
...
## Recent Changes
- 012-deep-security-review: Added Python 3.11+ (backend), TypeScript ~5.4 (frontend) + FastAPI, Pydantic 2, httpx, aiosqlite (backend); React 18, TanStack Query, Vite (frontend)
