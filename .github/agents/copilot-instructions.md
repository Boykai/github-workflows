...
- Python 3.11 (backend), TypeScript/React (frontend) + FastAPI, aiosqlite, Pydantic v2 (backend); React 18, Vite, shadcn/ui tokens (frontend) (012-mcp-settings-config)
- SQLite (WAL mode) via aiosqlite — new `mcp_configurations` table (migration 006) (012-mcp-settings-config)
...
## Recent Changes
- 012-mcp-settings-config: Added Python 3.11 (backend), TypeScript/React (frontend) + FastAPI, aiosqlite, Pydantic v2 (backend); React 18, Vite, shadcn/ui tokens (frontend)
