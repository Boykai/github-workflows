...
- TypeScript 5.x (frontend), Python 3.11+ (backend) + React 18.3 + Vitest 4.0 + Testing Library (frontend), FastAPI + Pytest 7.4 + pytest-cov (backend) (001-test-coverage-bugfix)
- N/A — no new storage; testing covers existing aiosqlite-backed services (001-test-coverage-bugfix)
...
## Recent Changes
- 001-test-coverage-bugfix: Added TypeScript 5.x (frontend), Python 3.11+ (backend) + React 18.3 + Vitest 4.0 + Testing Library (frontend), FastAPI + Pytest 7.4 + pytest-cov (backend)
