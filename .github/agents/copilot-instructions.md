...
## Active Technologies
- Python ≥3.12 (backend), TypeScript ~5.9 with React 19 (frontend) + FastAPI, Pydantic, aiosqlite, cryptography, slowapi, githubkit (backend); Vite, React, TanStack Query (frontend); nginx (reverse proxy) (001-security-review)
- SQLite via aiosqlite (encrypted at rest with Fernet from `cryptography`) (001-security-review)
