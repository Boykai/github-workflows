...
3. **Agent Completion Detection**: All agents create PRs via hierarchical branching (each agent branches from the issue's main branch). For `speckit.specify`, `speckit.plan`, and `speckit.tasks`, the polling service extracts `.md` files from the PR branch, posts them as GitHub Issue comments, and posts the `<agent-name>: Done!` marker. Child PRs are squash-merged into the main branch and their branches are deleted. The `speckit.implement` agent's completion is detected via PR timeline events (`copilot_work_finished`, `review_requested`), and its child PR is also merged into the main branch before transitioning to In Review.
