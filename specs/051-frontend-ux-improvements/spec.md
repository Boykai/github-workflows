...
**Feature Branch**: `051-frontend-ux-improvements`  
**Created**: 2026-03-19  
**Status**: Draft  
**Input**: User description: "Plan: Solune Frontend UX Improvements — six-phase plan covering toast notifications, chat markdown rendering, kanban drag-and-drop, skeleton loading states, global keyboard shortcuts, and independent quick wins"

## User Scenarios & Testing *(mandatory)*

### User Story 1 — Action Feedback via Toast Notifications (Priority: P1)

A user performs any mutation in Solune — saving settings, adding agents, deleting workflows, updating pipeline configurations, or managing chores. Today these actions complete silently with no visual confirmation. The user has no way to know whether the action succeeded or failed without manually verifying the result. With this improvement, every mutation surfaces a toast notification with the appropriate severity (success, error, warning, info) that auto-dismisses after a short duration. Toasts stack when multiple actions fire in quick succession, and they visually match the celestial theme.

**Why this priority**: Silent mutations are the single biggest UX gap. Users cannot trust the application when actions produce no feedback. This causes redundant re-submissions, increased support requests, and erosion of confidence. Because toast notifications apply to every workflow in the app, this delivers the broadest impact per unit of effort.
...
2. **Given** a user triggers a mutation that fails (e.g., network error while deleting a workflow), **When** the failure response is received, **Then** an error toast appears with a user-friendly message describing the failure and persists until the user dismisses it.
...
5. **Given** the maximum number of visible toasts is reached, **When** a new toast fires, **Then** the oldest toast auto-dismisses to make room for the new one.
...
**Why this priority**: Chat is the primary interaction surface for AI-assisted workflows. When AI responses are unreadable, users cannot extract value from the assistant's output. Code snippets — the most actionable part of many AI responses — become unusable without proper rendering. The required libraries are already installed, so this improvement requires only wiring, not new dependencies.
...
A user views the project board and wants to move an issue card from one status column to another (e.g., "To Do" → "In Progress"). Today the board is a static grid with no drag-and-drop capability. Users must open individual issues and change the status manually, breaking their workflow. With this improvement, users can drag issue cards between columns, the system updates the issue status via the backend API with an optimistic UI update, and visual feedback (column highlighting, ghost card overlay) guides the interaction.

**Why this priority**: The project board is central to issue management. Manual status changes require opening individual issues, which breaks flow and adds unnecessary friction. Drag-and-drop is an expected interaction pattern for kanban boards. The drag-and-drop library is already installed and used in the pipeline builder, so the pattern and infrastructure already exist.
...
5. **Given** a user drops a card on the same column it started in, **When** the drop event fires, **Then** the action is treated as a no-op — no API call, no toast, no state change.
6. **Given** a user is using keyboard navigation, **When** the user activates drag mode on a card, **Then** they can use arrow keys to move between columns and Enter/Space to drop.

---

### User Story 4 — Skeleton Loading States (Priority: P4)

A user navigates to a page that requires data loading (project board, agent list, chat history). Today all loading states show a generic spinning loader. With this improvement, content-shaped skeleton placeholders appear in place of the actual content during loading, reducing perceived load time and eliminating layout shift when data arrives.

**Why this priority**: Skeleton states are a proven UX pattern that reduce perceived wait time by 30–40%. They also prevent jarring layout shifts that disrupt reading position and visual continuity. This is a quality-of-life improvement that applies across the entire app. The existing celestial-shimmer animation can be reused for the sweep effect.
...
4. **Given** a route-level page transition occurs, **When** the entire page is loading via a suspense boundary, **Then** the full-page loader is used (not skeletons).
5. **Given** a user with a screen reader navigates during loading, **When** skeletons are visible, **Then** an appropriate announcement indicates that content is loading.
6. **Given** the loading state persists beyond 10 seconds, **When** skeletons are still showing, **Then** a subtle retry affordance or informational message appears to prevent the user from assuming the app is frozen.
...
**Why this priority**: Keyboard shortcuts are a quality-of-life feature for power users that increase efficiency and make the app feel professional. This builds on the existing command system and tooltip infrastructure.
...
3. **Given** a user is on any page and no text input is focused, **When** the user presses a number key `1` through `5`, **Then** the app navigates to the corresponding section (e.g., 1 = Dashboard, 2 = Board, 3 = Agents, 4 = Pipeline, 5 = Settings).
...
7. **Given** the chat panel is closed, **When** the user presses `Ctrl+K`, **Then** the chat panel opens and the input receives focus.

---

### User Story 6 — Quick Wins: Board Filtering, Tour Progress, Chat Polish, and Visual Enhancements (Priority: P6)

A collection of independent, low-to-medium impact improvements that polish the overall UX: board filtering/sorting by assignee, label, and priority; onboarding tour step progress indicator ("Step X of N"); chat date separators between messages from different days; notification bell pulse animation when unread count is greater than zero; empty state enrichment with celestial illustrations and suggested next steps; and `Ctrl+Enter` to send messages in chat.
...
2. **Given** a user is in the onboarding tour, **When** a spotlight tooltip is displayed, **Then** a progress indicator shows "Step X of N" (where X is the current step and N is the total number of steps).
...
7. **Given** board filters are active and no issues match, **When** the filtered view renders, **Then** a friendly empty state displays indicating no matches and offers a "Clear filters" action.
...
- How does markdown rendering handle malicious or deeply nested HTML in AI responses? HTML in markdown must be sanitized — raw HTML tags must not execute.
...
- What happens when board filtering returns zero results? A friendly empty state should display indicating no matches and offering a "Clear filters" action.
...
- **FR-020**: Route-level suspense boundaries MUST continue using the full-page loader.
- **FR-021**: Skeleton animations MUST use the existing celestial-shimmer effect for visual consistency.

**Global Keyboard Shortcuts**

- **FR-022**: Users MUST be able to press `?` (when no text input is focused) to open a keyboard shortcut help modal.
- **FR-023**: Users MUST be able to press `Ctrl+K` / `Cmd+K` to focus the chat input from any page, opening the chat panel if it is closed.
...
- **Skeleton Placeholder**: A loading-state placeholder shaped to match a specific content type (column, card, message, agent tile). Has dimensions, animation style, and an accessible loading announcement.
...
- The application uses a celestial visual theme with existing design tokens and keyframe animations (including celestial-shimmer) that new components will reuse.
- The markdown and GFM processing packages are already included in the project dependencies and do not need to be added.
- The drag-and-drop library is already included in the project dependencies and has an existing usage pattern in the pipeline builder that can be referenced.
- A lightweight toast library is the only new dependency required (Phase 1).
- The application uses a mutation-hook pattern (e.g., useSettingsForm, usePipelineConfig, useAgentConfig, useChores) where toast calls will be integrated.
- The project board columns map to issue status values that can be updated via the existing backend API.
- The sidebar has 5 major navigation sections that will map to number keys 1–5.
- The onboarding tour has a known total step count that can be displayed in the progress indicator.
- A 320px–1920px viewport range is the responsive design target for all new components.
