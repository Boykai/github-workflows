...
**Created**: 2026-03-07  
**Status**: Draft  
**Input**: User description: "On the app user chat, when the AI Enhance is disabled I receive an error. If the 'AI Enhance' is disabled, use the user's exact chat input for the GitHub Parent Issue description, generate a concise title for the fallback proposal, and add Agent Pipeline configuration information to description."

## User Scenarios & Testing *(mandatory)*

### User Story 1 — Submit Chat Request with AI Enhance Disabled (Priority: P1)

As a Solune app user, I want to submit a chat request with the AI Enhance toggle turned off and have the system create a GitHub Parent Issue using my exact chat input as the description body, while the Chat Agent generates a concise fallback proposal title and appends Agent Pipeline configuration to the description — without encountering the "I couldn't generate a task from your description" error.
...
**Independent Test**: Can be fully tested by toggling AI Enhance off, typing any non-empty message in the chat, submitting it, and verifying that a proposal is created with the raw user text as the proposed description and an AI-generated title.
...
2. **Given** the user has the AI Enhance toggle disabled and submits a chat request, **When** the fallback proposal pipeline executes, **Then** the Chat Agent generates a concise title and preserves the user's raw input as the proposal description.
...
As a project manager reviewing issues created by my team, I want issues confirmed from the AI Enhance disabled fallback path to use the same downstream creation flow as enhanced proposals — especially the same Agent Pipeline config section and confirmation behavior — so that my workflows, automations, and triage processes work consistently regardless of the enhancement setting.
...
**Independent Test**: Can be tested by creating two issues — one with AI Enhance on and one with AI Enhance off — from the same chat input, confirming both proposals, and comparing the resulting issue creation flow: both should append the same Agent Pipeline configuration section and use the same confirm-proposal plumbing.

**Acceptance Scenarios**:

1. **Given** a user creates a proposal with AI Enhance enabled and another with AI Enhance disabled using the same chat input, **When** both proposals are confirmed, **Then** both follow the same confirm-proposal flow to create the GitHub issue.
...
- What happens when the AI Enhance feature is disabled at a system/admin level vs. user toggle level? The system should treat both as equivalent — if enhancement is off for any reason, the fallback title-only path should activate.

## Requirements *(mandatory)*

### Functional Requirements

- **FR-001**: System MUST detect when the AI Enhance toggle is disabled before invoking the content generation pipeline and branch into the fallback title-only path.
...
- **FR-003**: System MUST invoke the Chat Agent to generate the proposal title when AI Enhance is disabled, while preserving the user's raw input as the proposal description in the generic task fallback path.
...
- **FR-007**: System SHOULD validate that the Chat Agent title-generation step succeeded independently of the AI Enhance step, and surface a specific, actionable error only if title generation or fallback proposal creation fails.
- **FR-008**: System MUST ensure the final confirmation flow for proposals created with AI Enhance disabled reuses the same downstream issue-creation and Agent Pipeline configuration logic as proposals created with AI Enhance enabled.
...
- **AI Enhance Toggle**: A user-facing setting that controls whether the content enhancement pipeline is applied to the chat input. States: enabled (full AI pipeline) or disabled (title-only fallback path).
- **GitHub Parent Issue**: The resulting issue created on GitHub after proposal confirmation. Composed of the chosen title, description body (raw user input or AI-enhanced content), and the appended Agent Pipeline configuration block from the existing confirmation flow.
...
- **Chat Agent**: The AI component responsible for generating fallback proposal titles and, in other paths, richer issue recommendations from the user's chat input. Operates independently of the AI Enhance content generation step.
...
- **SC-003**: The fallback proposal created with AI Enhance disabled always contains an AI-generated title plus the user's verbatim input as the proposed description, with no regressions in the existing feature-request recommendation path.
...
- **SC-006**: Issues created from AI Enhance disabled proposals pass the same downstream confirmation and Agent Pipeline configuration flow as AI Enhance–enabled proposals.
...
- The Chat Agent is capable of generating a concise title from raw user input without requiring AI-enhanced content as input.
...
- The issue creation pipeline can be refactored into two explicit branches (full pipeline vs. title-only fallback) without affecting existing AI Enhance–enabled behavior.
