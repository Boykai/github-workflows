...
- [x] T001 Update measurement workflow, representative board assumptions, and command checklist in `specs/034-performance-review/quickstart.md`
- [x] T002 [P] Add backend performance assertion helpers for API-call and cache-hit comparisons in `backend/tests/helpers/assertions.py`
- [x] T003 [P] Add frontend render-count and query-client test helpers in `frontend/src/test/test-utils.tsx`
...
- [x] T004 Record Spec 022 audit checkpoints and first-pass scope boundaries in `specs/034-performance-review/research.md`
- [x] T005 [P] Add board endpoint and cache measurement logging hooks in `backend/src/api/board.py`
- [x] T006 [P] Add websocket idle-cycle and refresh-cycle measurement logging hooks in `backend/src/api/projects.py`
- [x] T007 [P] Centralize board and task query-key helpers for selective invalidation in `frontend/src/hooks/useProjectBoard.ts`
...
- [x] T008 [P] [US1] Extend backend baseline coverage for idle board endpoint activity in `backend/tests/unit/test_api_board.py`
- [x] T009 [P] [US1] Extend frontend baseline profiling coverage for representative board renders in `frontend/src/pages/ProjectsPage.test.tsx`

### Implementation for User Story 1

- [x] T010 [US1] Document backend baseline measurements and observation steps in `specs/034-performance-review/quickstart.md`
- [x] T011 [US1] Document frontend baseline measurements and Spec 022 audit findings in `specs/034-performance-review/research.md`
- [x] T012 [US1] Capture baseline metric tables and pre-optimization thresholds in `specs/034-performance-review/contracts/board-endpoints.md`
...
- [x] T013 [P] [US2] Extend warm-cache and forced-refresh coverage in `backend/tests/unit/test_api_board.py`
- [x] T014 [P] [US2] Extend websocket zero-change cycle coverage in `backend/tests/unit/test_api_projects.py`
- [x] T015 [P] [US2] Extend idle polling no-full-refresh coverage in `backend/tests/unit/test_copilot_polling.py`

### Implementation for User Story 2

- [x] T016 [US2] Clear board and related sub-issue cache keys on manual refresh in `backend/src/api/board.py`
- [x] T017 [US2] Reuse warm sub-issue cache entries before refetching GitHub data in `backend/src/services/github_projects/service.py`
- [x] T018 [US2] Skip unchanged websocket refresh cycles before expensive board fetches in `backend/src/api/projects.py`
- [x] T019 [US2] Gate fallback polling escalation behind lightweight change detection in `backend/src/services/copilot_polling/polling_loop.py`
- [x] T020 [P] [US2] Replace duplicate repository resolution calls with the canonical helper in `backend/src/api/workflow.py`
- [x] T021 [P] [US2] Memoize repeated repository resolution results for refresh flows in `backend/src/utils.py`
...
- [x] T022 [P] [US3] Extend task-only invalidation coverage for websocket and polling events in `frontend/src/hooks/useRealTimeSync.test.tsx`
- [x] T023 [P] [US3] Extend auto-refresh and manual-refresh policy coverage in `frontend/src/hooks/useBoardRefresh.test.tsx`
- [x] T024 [P] [US3] Extend board and task query-key contract coverage in `frontend/src/hooks/useProjectBoard.test.tsx`

### Implementation for User Story 3

- [x] T025 [US3] Narrow fallback polling invalidation to task-only query keys in `frontend/src/hooks/useRealTimeSync.ts`
- [x] T026 [US3] Align auto-refresh and manual-refresh paths to the shared refresh policy in `frontend/src/hooks/useBoardRefresh.ts`
- [x] T027 [US3] Export stable board and task query-key helpers for refresh callers in `frontend/src/hooks/useProjectBoard.ts`
...
- [x] T028 [P] [US4] Extend derived-state and rerender-scope coverage in `frontend/src/pages/ProjectsPage.test.tsx`
- [x] T029 [P] [US4] Extend sibling-column rerender guards in `frontend/src/components/board/BoardColumn.test.tsx`
- [x] T030 [P] [US4] Extend unchanged-card rerender guards in `frontend/src/components/board/IssueCard.test.tsx`

### Implementation for User Story 4

- [x] T031 [US4] Memoize pipeline grid style and other stable board-derived values in `frontend/src/pages/ProjectsPage.tsx`
- [x] T032 [US4] Stabilize modal and popover listener callbacks to reduce hot re-registration in `frontend/src/pages/ProjectsPage.tsx`
- [x] T033 [US4] Preserve memoized drag-update boundaries for columns in `frontend/src/components/board/BoardColumn.tsx`
- [x] T034 [US4] Preserve memoized single-card update boundaries in `frontend/src/components/board/IssueCard.tsx`
...
- [x] T035 [P] [US5] Extend backend cache regression coverage for warm-versus-cold sub-issue behavior in `backend/tests/unit/test_cache.py`
- [x] T036 [P] [US5] Extend frontend end-to-end refresh regression coverage in `frontend/src/hooks/useBoardRefresh.test.tsx`

### Implementation for User Story 5

- [x] T037 [US5] Record backend regression-suite commands and manual verification steps in `specs/034-performance-review/quickstart.md`
- [x] T038 [US5] Record frontend regression-suite results and manual profile pass notes in `specs/034-performance-review/quickstart.md`
- [x] T039 [US5] Reconcile measured results against SC-001 through SC-009 in `specs/034-performance-review/research.md`
...
- [x] T040 [P] Align verified cache reuse and forced-refresh behavior notes in `specs/034-performance-review/contracts/backend-cache.md`
- [x] T041 [P] Align verified endpoint and rerender measurement notes in `specs/034-performance-review/contracts/board-endpoints.md`
- [x] T042 [P] Align verified unified refresh policy notes in `specs/034-performance-review/contracts/refresh-policy.md`
- [x] T043 [P] Document deferred second-wave follow-up items only (no implementation) in `specs/034-performance-review/research.md`
- [x] T044 [P] Refresh final implementation checklist, command matrix, and validation notes in `specs/034-performance-review/quickstart.md`
