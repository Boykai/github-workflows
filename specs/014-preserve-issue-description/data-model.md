...
The user-provided issue description is not a standalone entity in the data model. It flows through existing entities as an **immutable string** from input to API delivery.

| Stage | Entity | Field(s) | Max Length | Mutable? |
|-------|--------|----------|------------|----------|
| Input | `ChatMessageRequest` | `content` | Unbounded | No |
| AI Processing | `IssueRecommendation` | `original_input`, `original_context` | Unbounded | No |
| AI-Generated Fields | `IssueRecommendation` | `user_story`, `ui_ux_description`, `functional_requirements`, `technical_notes` | Unbounded | No |
