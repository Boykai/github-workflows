# Data Model: Performance Review — Balanced First Pass
...
**Behavioral changes**:
- Automatic board refreshes MUST check sub-issue cache before making per-issue REST calls (already implemented in `service.py`)
- Only issues with expired or missing cache entries trigger new API calls
- Manual refresh clears all sub-issue cache entries for the project before fetching (already implemented in `board.py`)
...
### 3. Refresh Event (conceptual, no persistence)

Represents a trigger to update board data. Used to define the refresh contract between backend and frontend. This is a conceptual entity — not stored in any database or cache.
...
### 4. Performance Baseline (documentation only)
...
| single_card_update_latency | ms | User-perceived time from WebSocket update to UI reflect | <2000ms |
...
- Reconnection debounce window: 2 seconds (already implemented in `useRealTimeSync.ts`)
...
| Reconnection must not cascade invalidations | WebSocket State | At most 1 task invalidation per reconnection cycle (2s debounce) |
