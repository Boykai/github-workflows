...
- **FR-009**: Pagination MUST work in combination with client-side filtering — when a user applies a search or filter, the paginated results must reflect the filtered data set.
- **FR-010**: Pagination MUST work in combination with client-side sorting — when a user changes the sort order, the list must reset to the first page with the new sort applied.
...
- Existing client-side filtering and sorting continue to operate on the loaded data set; server-side filtering and sorting are not in scope for this feature unless pagination makes them necessary for correctness.
