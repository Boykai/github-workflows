...
- [ ] T001 Install `@radix-ui/react-tooltip` dependency in frontend/package.json
- [ ] T002 Create `frontend/src/constants/` directory for static data modules
...
- [ ] T003 Create `TooltipEntry` interface and centralized tooltip content registry with all ~38 entries organized by area (board, chat, agents, pipeline, chores, settings, tools) in frontend/src/constants/tooltip-content.ts
- [ ] T004 Create reusable `Tooltip` wrapper component built on `@radix-ui/react-tooltip` with `contentKey` and `content` props, theme-aware styling using existing CSS custom properties (`--popover`, `--popover-foreground`, `--border`), directional arrow, max-width 280px, min font size 13px, 300ms delay, portal rendering, graceful fallback for missing keys, and `prefers-reduced-motion` support in frontend/src/components/ui/tooltip.tsx
- [ ] T005 Re-export `TooltipProvider` from the Tooltip component module for use in App.tsx in frontend/src/components/ui/tooltip.tsx
- [ ] T006 Wrap application root with `<TooltipProvider delayDuration={300} skipDelayDuration={300}>` around the existing `QueryClientProvider` in frontend/src/App.tsx
...
- [ ] T007 [P] [US1] Wrap refresh button with `<Tooltip contentKey="board.toolbar.refreshButton">` in frontend/src/components/board/RefreshButton.tsx
- [ ] T008 [P] [US1] Wrap clean-up button with `<Tooltip contentKey="board.toolbar.cleanUpButton">` in frontend/src/components/board/CleanUpButton.tsx
- [ ] T009 [P] [US1] Wrap filter, sort, and group controls with `<Tooltip>` using appropriate content keys in frontend/src/components/board/BoardToolbar.tsx
- [ ] T010 [P] [US1] Wrap AI Enhance toggle, attach, voice, and send buttons with `<Tooltip>` using appropriate content keys in frontend/src/components/chat/ChatToolbar.tsx
- [ ] T011 [P] [US1] Wrap history toggle and message actions with `<Tooltip>` using appropriate content keys in frontend/src/components/chat/ChatInterface.tsx
- [ ] T012 [P] [US1] Wrap edit, delete, and model selector actions with `<Tooltip>` using appropriate content keys in frontend/src/components/agents/AgentCard.tsx
- [ ] T013 [P] [US1] Wrap search input, sort, bulk update, and add agent controls with `<Tooltip>` using appropriate content keys in frontend/src/components/agents/AgentsPanel.tsx
- [ ] T014 [P] [US1] Wrap system prompt field, tools editor, and model selection fields with `<Tooltip>` using appropriate content keys in frontend/src/components/agents/AddAgentModal.tsx
- [ ] T015 [P] [US1] Wrap model selector and delete button with `<Tooltip>` using appropriate content keys in frontend/src/components/pipeline/StageCard.tsx
- [ ] T016 [P] [US1] Wrap add stage, save, and delete pipeline buttons with `<Tooltip>` using appropriate content keys in frontend/src/components/pipeline/PipelineBoard.tsx
- [ ] T017 [P] [US1] Wrap model dropdown with `<Tooltip>` using appropriate content key in frontend/src/components/pipeline/ModelSelector.tsx
- [ ] T018 [P] [US1] Wrap execute, edit, delete, and AI Enhance toggle with `<Tooltip>` using appropriate content keys in frontend/src/components/chores/ChoreCard.tsx
- [ ] T019 [P] [US1] Wrap theme toggle and model management controls with `<Tooltip>` using appropriate content keys in frontend/src/pages/SettingsPage.tsx and frontend/src/components/settings/ components
- [ ] T020 [P] [US1] Wrap tool configure and status toggle controls with `<Tooltip>` using appropriate content keys in frontend/src/pages/ToolsPage.tsx and frontend/src/components/tools/ToolCard.tsx
...
- [ ] T021 [US2] Verify and tune tooltip visual design consistency — ensure all tooltip instances render with `bg-popover`, `text-popover-foreground`, `border-border/60`, `max-w-[280px]`, `text-[13px]`, `rounded-lg`, `shadow-md`, and directional arrow across both light and dark themes in frontend/src/components/ui/tooltip.tsx
- [ ] T022 [US2] Verify intelligent positioning — confirm Radix Tooltip `side` and `align` props produce correct flip behavior when tooltips are triggered near top, bottom, left, and right viewport edges; adjust `sideOffset` and `collisionPadding` values if needed in frontend/src/components/ui/tooltip.tsx
- [ ] T023 [US2] Verify tooltips do not obstruct adjacent interactive elements — ensure z-index (50), portal rendering, and spacing prevent overlap with nearby clickable elements; adjust `sideOffset` or `collisionPadding` if needed in frontend/src/components/ui/tooltip.tsx
...
- [ ] T024 [US3] Verify Radix Tooltip provides `aria-describedby` on trigger elements and `role="tooltip"` on content automatically; add any missing ARIA attributes if needed in frontend/src/components/ui/tooltip.tsx
- [ ] T025 [US3] Audit keyboard navigation across all pages — confirm tooltips appear when interactive elements receive keyboard focus via Tab and dismiss on Escape or focus loss; fix any focus-related issues in frontend/src/components/ui/tooltip.tsx
...
- [ ] T027 [US4] Implement progressive disclosure rendering in the Tooltip component — render `title` field as bold `<p className="font-semibold">` heading above summary, render `learnMoreUrl` as a "Learn more →" link below summary, skip title/link when fields are absent in frontend/src/components/ui/tooltip.tsx
- [ ] T028 [US4] Add `title` and `learnMoreUrl` fields to complex feature tooltip entries in the content registry — ensure entries for `agents.modal.systemPrompt`, `agents.modal.toolsEditor`, `agents.card.modelSelector`, `agents.panel.bulkUpdateButton`, `pipeline.stage.modelSelector`, `pipeline.board.savePipelineButton`, `chores.card.executeButton`, `chores.card.aiEnhanceToggle`, and `chat.toolbar.aiEnhanceToggle` include a bold title and optional learn more URL in frontend/src/constants/tooltip-content.ts
- [ ] T029 [US4] Verify simple tooltip entries (e.g., `board.toolbar.refreshButton`, `agents.card.deleteButton`) in frontend/src/constants/tooltip-content.ts display only summary text without title or "Learn more" link
...
- [ ] T030 [US5] Audit all instrumented component files to ensure no inline tooltip text is hardcoded — all tooltips must use `contentKey` prop referencing the centralized registry in frontend/src/constants/tooltip-content.ts
- [ ] T031 [US5] Verify registry key naming consistency — all keys follow the `{area}.{section}.{element}` dot-notation convention and each key uniquely maps to one UI element in frontend/src/constants/tooltip-content.ts
- [ ] T032 [US5] Verify `TooltipEntry` type is exported from the tooltip component module so the registry can import it for type safety in frontend/src/components/ui/tooltip.tsx and frontend/src/constants/tooltip-content.ts
...
- [ ] T033 [P] Verify `prefers-reduced-motion` media query support — tooltip appear/disappear animations should be disabled or reduced when the user has motion reduction enabled, using Tailwind's `motion-reduce:animate-none` variant in frontend/src/components/ui/tooltip.tsx
- [ ] T034 [P] Verify rapid cursor scanning behavior — moving quickly between tooltip triggers should dismiss the previous tooltip and show the next one without delay lag (leveraging `skipDelayDuration={300}` on `TooltipProvider` configured in frontend/src/App.tsx)
- [ ] T035 [P] Verify graceful handling of missing registry keys — hovering over an element with a `contentKey` that has no entry in the registry should render children only (no empty or broken tooltip) and log a `console.warn` in development mode in frontend/src/components/ui/tooltip.tsx
- [ ] T036 Run `npx tsc --noEmit` to confirm zero TypeScript errors after all tooltip changes
- [ ] T037 Run `npx vitest run` to confirm all existing tests pass with no regressions from tooltip changes
- [ ] T038 Run quickstart.md validation — follow all 16 verification steps from specs/030-ui-tooltips/quickstart.md to confirm end-to-end tooltip behavior
...
- All US1 component integration tasks (T007–T020) can run in parallel — each modifies a different file
- US2, US3, and US4 foundational work (T021, T024, T027) can run in parallel with US1 component integration
...
Task T007: Wrap refresh button in frontend/src/components/board/RefreshButton.tsx
Task T008: Wrap clean-up button in frontend/src/components/board/CleanUpButton.tsx
Task T009: Wrap board toolbar controls in frontend/src/components/board/BoardToolbar.tsx
Task T010: Wrap chat toolbar controls in frontend/src/components/chat/ChatToolbar.tsx
Task T011: Wrap chat interface controls in frontend/src/components/chat/ChatInterface.tsx
Task T012: Wrap agent card actions in frontend/src/components/agents/AgentCard.tsx
Task T013: Wrap agents panel controls in frontend/src/components/agents/AgentsPanel.tsx
Task T014: Wrap add agent modal fields in frontend/src/components/agents/AddAgentModal.tsx
Task T015: Wrap stage card controls in frontend/src/components/pipeline/StageCard.tsx
Task T016: Wrap pipeline board actions in frontend/src/components/pipeline/PipelineBoard.tsx
Task T017: Wrap model selector in frontend/src/components/pipeline/ModelSelector.tsx
Task T018: Wrap chore card actions in frontend/src/components/chores/ChoreCard.tsx
Task T019: Wrap settings controls in frontend/src/pages/SettingsPage.tsx
Task T020: Wrap tools controls in frontend/src/pages/ToolsPage.tsx
```
...
- Setup: 2 tasks, Foundational: 4 tasks, Polish: 6 tasks
