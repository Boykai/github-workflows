...
- The audit scope covers the Chores page (`ChoresPage.tsx`) and all components rendered within it — `ChoresPanel`, `ChoreCard`, `AddChoreModal`, `ChoreScheduleConfig`, `ChoreChatFlow`, `ChoreInlineEditor`, `ConfirmChoreModal`, `FeaturedRitualsPanel`, `PipelineSelector`, and `CelestialCatalogHero` (shared component used on this page) — but does not extend to shared layout elements (navigation, sidebar) unless they exhibit issues unique to the Chores page context.
