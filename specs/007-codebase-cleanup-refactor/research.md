...
**Decision**: Convert `copilot_polling.py` (3,988 lines) into a `copilot_polling/` Python package with 9 sub-modules plus a re-export `__init__.py` facade.
...
- *Keep as single file with better organization* — Rejected: 3,988 lines is unmanageable.
...
**Decision**: Standardize on the existing `AppException` hierarchy in `exceptions.py`. Replace all direct `HTTPException` raises with `AppException` subclasses. Update the generic exception handler to log full details server-side while returning sanitized responses.
...
- Rate limiter in `main.py` uses `HTTPException(429)` instead of `RateLimitError`
- `github_projects.py` has ~30+ bare `except Exception` blocks returning `False`/`None`/`[]`
...
| `300000` ms | `useAuth.ts` | `AUTH_STALE_TIME_MS` |
| `10000` ms | `useChat.ts`, `useProjectBoard.ts` | `DATA_STALE_TIME_MS` |

## R6. Frontend API Client Enhancement

**Decision**: Enhance the existing `api.ts` `request<T>()` function. No new library needed.
...
**Changes**:
1. Add centralized error interceptor to `request<T>()`: on 401, clear auth cache and redirect
2. Add network error handling (catch `TypeError` from failed `fetch`)
3. Verify no raw `fetch()` calls exist outside `api.ts` (confirmed: none found)

**Alternatives considered**:
- *Replace with axios* — Rejected: Adding a dependency for marginal benefit.
- *Full API client class* — Rejected: Function-based approach is simpler.

## R7. WebSocket/Polling Unification

**Decision**: Make `useRealTimeSync` the single source of real-time updates. Remove the independent 15s polling from `useProjectBoard`.
...
2. `useRealTimeSync` invalidates board-related query keys on WS events
3. WS disconnect activates polling fallback (already implemented, 5s)
...
**Implementation**:
- `getPersistedState<T>(key): T | null` + `setPersistedState<T>(key, value): void`
- Hooks write on state change; `App.tsx` reads on mount
...
**Alternatives considered**:
- *react-error-boundary library* — Rejected: Standard React API is sufficient.

## R10. Test Coverage Strategy

**Decision**: Add unit tests for 5 untested backend services and 6 untested frontend hooks using existing test infrastructure.

**Backend** (pytest + pytest-asyncio): Follow patterns from `test_github_projects.py` and `test_ai_agent.py` — mock external dependencies, test core logic.

**Frontend** (Vitest + @testing-library/react): Follow patterns from `useAuth.test.tsx` — mock API module, test hook state transitions.

**Note**: `test_workflow_orchestrator.py` already exists (958 lines). Only `completion_providers`, `session_store`, `settings_store`, and `agent_tracking` need new test files.

## R11. `workflow_orchestrator.py` Reduction

**Decision**: At 1,959 lines, this file will reduce after extracting shared patterns (pipeline state helpers, cache keys) but may remain over 500 lines. If substantially over, split following the same package pattern. The orchestrator's complexity is inherent (multi-step workflow coordination), so a module at ~800 lines with clear single responsibility is acceptable per the spec's Assumptions section.
