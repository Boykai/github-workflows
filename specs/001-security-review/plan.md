...
Comprehensive security audit covering 21 findings across OWASP Top 10 categories (3 Critical, 8 High, 9 Medium, 2 Low). The audit addresses session management, encryption enforcement, container security, authorization controls, HTTP hardening, rate limiting, data privacy, and configuration validation. Changes span the backend (FastAPI/Python), frontend (React/TypeScript), nginx reverse proxy, Docker containers, and CI/CD workflows. All findings have corresponding research decisions, data model updates, behavioral contracts, and a phased quickstart guide.

## Technical Context

**Language/Version**: Python ≥3.12 (backend), TypeScript ~5.9.0 (frontend)
**Primary Dependencies**: FastAPI ≥0.135.0, React 19.2, nginx 1.29-alpine, slowapi ≥0.1.9, cryptography ≥46.0.5
...
**Post-Design Re-check**: All principles remain satisfied. The `repo` OAuth scope retention is justified with a code comment explaining GitHub API requirements (see research.md Decision 8). No new abstractions introduced — all changes modify existing files using established patterns.
...
│   │       ├── encryption.py    # FR-003: Encryption with mandatory key
...
**Structure Decision**: Web application (backend + frontend). All security changes are scoped to existing files — no new directories or modules required. Changes modify existing patterns in place.

## Complexity Tracking

> No constitution violations identified. One scope decision requires justification:

| Violation | Why Needed | Simpler Alternative Rejected Because |
|-----------|------------|-------------------------------------|
| `repo` OAuth scope retained (FR-013) | GitHub API returns misleading 404s for issue/PR creation without `repo` scope | Narrower scopes (`public_repo`, `project`) were tested and do not support the core workflow (creating issues, sub-issues, comments, labels, and PRs). Migration to GitHub App installation tokens deferred to future enhancement. |
