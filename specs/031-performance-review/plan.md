...
**Branch**: `031-performance-review` | **Date**: 2026-03-08 | **Spec**: [spec.md](spec.md)
**Input**: Feature specification from `/specs/031-performance-review/spec.md`
...
Feature work began with explicit specification (spec.md) containing 7 prioritized user stories (P1–P3), Given-When-Then acceptance scenarios for each, clear scope boundaries (board virtualization, major refactors, new dependencies out of scope), edge case analysis, and 11 measurable success criteria (SC-001 through SC-011).
...
| IV. Test Optionality | ✅ PASS | Regression tests required by spec (FR-015); new tests only for modified areas |
...
├── src/
│   ├── api/
│   │   ├── board.py             # FR-005,006,007,008: Sub-issue cache, manual refresh bypass
│   │   ├── projects.py          # FR-003,012: WebSocket change detection, reconnection guard
│   │   └── workflow.py          # Repository resolution consolidation candidate
│   ├── services/
│   │   ├── cache.py             # FR-007,008: Sub-issue cache TTL, bounded cache
│   │   ├── copilot_polling/
│   │   │   └── polling_loop.py  # FR-004,009: Polling-triggered refresh guard
│   │   └── github_projects/
│   │       └── service.py       # FR-007: Sub-issue caching in fetch path
...
├── src/
│   ├── hooks/
│   │   ├── useRealTimeSync.ts       # FR-005,012,014: Decouple task updates from board query
│   │   ├── useBoardRefresh.ts       # FR-006,013,014: Manual refresh priority, dedup
│   │   └── useProjectBoard.ts       # FR-005: Board query invalidation strategy
│   ├── components/
│   │   ├── board/
│   │   │   ├── BoardColumn.tsx      # FR-009: React.memo, stable props
│   │   │   └── IssueCard.tsx        # FR-009: React.memo, stable props
│   │   ├── chat/
│   │   │   └── ChatPopup.tsx        # FR-011: Throttle drag event listeners
│   │   └── agents/
│   │       └── AddAgentPopover.tsx   # FR-011: Memoize updatePosition callback
│   └── pages/
│       └── ProjectsPage.tsx         # FR-010: useMemo for sorting/aggregation
└── tests/
    └── hooks/
...
**Structure Decision**: Existing web application layout (backend/ + frontend/). No new directories or files beyond the contracts/ subdirectory. All changes are in-place modifications to existing files using standard patterns already present in the codebase.
