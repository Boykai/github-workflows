...
### 1. Current Backend State vs. Spec 022 (FR-013)

- **Decision**: Spec 022 is substantially implemented. WebSocket change detection (hash comparison), board cache TTL alignment (300s), and sub-issue cache invalidation on manual refresh are all present. Remaining gaps are limited to: (a) fallback polling still potentially triggering board data refreshes, (b) sub-issue cache could benefit from tighter warm-cache reuse on automatic refreshes.
- **Rationale**: Code inspection of `backend/src/api/projects.py` confirms SHA256 hash comparison on each 30-second WebSocket cycle — refresh messages are only sent when data actually changes (FR-001/FR-002 of Spec 022). `backend/src/api/board.py` confirms a 300-second cache TTL for board data (matching the frontend 5-minute auto-refresh interval) and pre-clears sub-issue caches on manual refresh. The remaining work is incremental tightening, not reimplementation.
- **Alternatives considered**:
  ...
  - Skip verification entirely — rejected because the spec requires explicit confirmation before optimization work begins.
- **Key evidence**:
  ...
  - `polling_loop.py`: Adaptive backoff (60→120→240→300s cap after idle cycles); rate-limit pre-checks; sub-issues filtered out upfront

### 2. WebSocket Reconnection and Refresh Cascades (FR-010)
...
### 3. Fallback Polling and Board Data Refresh (FR-003, FR-009)
...
### 4. Frontend Component Memoization Strategy (FR-008)

- **Decision**: Apply `React.memo()` to `BoardColumn` and `IssueCard` components with shallow prop comparison. Stabilize callback props (`onCardClick`) in the parent component using `useCallback`. Do not add custom deep comparators — shallow comparison is sufficient given the data flow.
- **Rationale**: Code inspection shows neither `BoardColumn` nor `IssueCard` is wrapped in `React.memo()`. Every parent re-render (triggered by any state change in ProjectsPage) causes all columns and all cards to re-render. For a 100-card board across 5 columns, this means 100+ unnecessary re-renders on a single-card status change. `React.memo()` with stable props will skip re-rendering unchanged components.
- **Alternatives considered**:
...
- **Implementation approach**:
  - Wrap `BoardColumn` export with `React.memo(BoardColumn)`
  - Wrap `IssueCard` export with `React.memo(IssueCard)`
...
### 5. Derived State Computation in ProjectsPage (FR-014)
...
### 6. Event Listener Throttling for ChatPopup and AddAgentPopover (FR-009)
...
### 7. Sub-Issue Cache Warm Reuse on Automatic Refresh (FR-006)
...
### 8. Manual Refresh Priority and Deduplication (FR-005, FR-011)
...
- **Rationale**: The spec requires that concurrent refresh requests are deduplicated with manual refresh taking priority. If an automatic refresh is in-flight when the user clicks manual refresh, both requests could complete and cause unnecessary state updates. TanStack Query's `cancelQueries` provides the standard mechanism for this.
- **Alternatives considered**:
...
- **Rationale**: The spec mandates baseline measurements before any code changes (FR-001, Story 6). The protocol must be repeatable so post-optimization measurements use the same methodology.
- **Alternatives considered**:
