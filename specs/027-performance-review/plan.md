...
**Branch**: `027-performance-review` | **Date**: 2026-03-07 | **Spec**: [spec.md](spec.md)
**Input**: Feature specification from `/specs/027-performance-review/spec.md`

## Summary

Deliver a balanced, measurement-driven first pass of performance optimizations across backend and frontend. Backend work targets remaining GitHub API churn around board refreshes and polling — verifying Spec 022 completion, tightening WebSocket change-detection refresh semantics, warming sub-issue caches, and eliminating fallback-polling-triggered board refreshes. Frontend work targets board responsiveness — decoupling lightweight task updates from the expensive board query, memoizing high-rerender components (BoardColumn, IssueCard), stabilizing derived-state computation in ProjectsPage, and throttling hot event listeners in ChatPopup and AddAgentPopover. All changes are gated on baseline measurements captured before any optimization code lands, and validated with before/after comparisons.
...
**Performance Goals**: ≤2 idle API calls/5 min, <1s perceived single-card update, ≥30 fps board drag, ≤3 component re-renders per single-card change, ≥80% sub-issue cache hit rate on automatic refresh
...
**Scale/Scope**: ~15 backend files modified, ~10 frontend files modified; target board size 50–200 cards across 3–7 columns
...
The spec mandates baseline measurement (FR-001) and regression coverage (FR-012) as functional requirements. New unit tests are added only for areas directly modified by optimization changes (cache behavior, refresh hooks, component memoization). Existing test suites serve as regression gates. Tests are mandatory where the spec explicitly requires them, optional otherwise.
...
|-----------|--------|-------|
| I. Specification-First | ✅ PASS | All 14 FRs traced to design artifacts |
...
├── src/
│   ├── api/
│   │   ├── board.py             # FR-005,006,007: Sub-issue cache, manual refresh bypass
│   │   └── projects.py          # FR-002,010: WebSocket change detection, reconnection guard
│   ├── services/
│   │   ├── cache.py             # FR-006,007: Sub-issue cache TTL, bounded cache
│   │   ├── copilot_polling/
│   │   │   └── polling_loop.py  # FR-003,009: Polling-triggered refresh guard
│   │   └── github_projects/
│   │       └── service.py       # FR-006: Sub-issue caching in fetch path
│   └── utils.py                 # Shared repository resolution (no changes expected)
└── tests/
    └── unit/
        ├── test_cache.py            # FR-012: Cache TTL regression tests
        ├── test_api_board.py        # FR-012: Board cache regression tests
        └── test_copilot_polling.py  # FR-012: Polling regression tests

frontend/
├── src/
│   ├── hooks/
│   │   ├── useRealTimeSync.ts       # FR-004,010: Decouple task updates from board query
│   │   ├── useBoardRefresh.ts       # FR-005,011: Manual refresh priority, dedup
│   │   └── useProjectBoard.ts       # FR-004: Board query invalidation strategy
│   ├── components/
│   │   ├── board/
│   │   │   ├── BoardColumn.tsx      # FR-008: React.memo, stable props
│   │   │   └── IssueCard.tsx        # FR-008: React.memo, stable props
│   │   ├── chat/
│   │   │   └── ChatPopup.tsx        # FR-009: Throttle drag event listeners
│   │   └── agents/
│   │       └── AddAgentPopover.tsx   # FR-009: Memoize updatePosition callback
│   └── pages/
│       └── ProjectsPage.tsx         # FR-014: useMemo for sorting/aggregation
└── tests/
    └── hooks/
        ├── useRealTimeSync.test.tsx  # FR-012: Refresh invalidation regression tests
        └── useBoardRefresh.test.tsx  # FR-012: Refresh timer regression tests
```

**Structure Decision**: Existing web application layout (backend/ + frontend/). No new directories or files. All changes are in-place modifications to existing files using standard patterns already present in the codebase.
