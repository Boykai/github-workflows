...
**Tests**: Tests are REQUIRED by the feature specification (FR-003: "at least one new regression test per bug"). Regression tests are included within each user story phase.

**Organization**: Tasks are grouped by user story (bug category) to enable independent implementation and testing of each category in strict priority order per FR-002.
...
See `specs/001-bug-basher/contracts/process-contracts.md` for the full contract.
...
- [ ] T008 Review backend service inventory — catalog all 33 service files in `solune/backend/src/services/` and 7 sub-packages (`agents/`, `chores/`, `copilot_polling/`, `github_projects/`, `pipelines/`, `tools/`, `workflow_orchestrator/`) to identify high-risk modules for each bug category
- [ ] T009 [P] Review backend API route inventory — catalog all 21 route files in `solune/backend/src/api/` and map input validation patterns
- [ ] T010 [P] Review middleware inventory — catalog all 5 middleware files in `solune/backend/src/middleware/` (admin_guard.py, csp.py, csrf.py, rate_limit.py, request_id.py) and identify security-relevant logic
...
- [ ] T013 Review existing test coverage — catalog 144 unit tests in `solune/backend/tests/unit/`, 15 integration tests in `solune/backend/tests/integration/`, and 152 frontend tests to identify gaps
...
- [ ] T025 [US1] Audit all 21 API route files in `solune/backend/src/api/` for missing input validation — verify Pydantic model usage on all endpoints in `activity.py`, `agents.py`, `apps.py`, `auth.py`, `board.py`, `chat.py`, `chores.py`, `cleanup.py`, `health.py`, `mcp.py`, `metadata.py`, `onboarding.py`, `pipelines.py`, `projects.py`, `settings.py`, `signal.py`, `tasks.py`, `tools.py`, `webhook_models.py`, `webhooks.py`, `workflow.py` — fix confirmed bugs, add regression tests
...
- [x] T031 [US1] For each ambiguous security finding, add `# TODO(bug-bash):` comment per contract format in `specs/001-bug-basher/contracts/process-contracts.md` — document issue, options, and rationale for deferral
...
- [ ] T072 [P] [US4] Identify untested code paths in `solune/backend/src/middleware/` — ensure `admin_guard.py`, `csp.py`, `csrf.py`, `rate_limit.py`, `request_id.py` have adequate error path coverage — add missing tests
...
- [ ] T095 Compile the Bug Report Summary Table per contract in `specs/001-bug-basher/contracts/process-contracts.md` — list every bug found with sequential number, file path, line(s), category, description, and status (✅ Fixed or ⚠️ Flagged)
- [ ] T096 Verify commit messages follow the contract format `fix(<category>): <short description>` per `specs/001-bug-basher/contracts/process-contracts.md`
...
- **US1: Security (Phase 3)**: Depends on Foundational completion — MUST complete before any other user story per FR-002
- **US2: Runtime (Phase 4)**: Depends on US1 completion (FR-002: no lower-priority fix while known security issues remain)
- **US3: Logic (Phase 5)**: Depends on US2 completion (FR-002 priority ordering)
...
- **US1 (P1)**: MUST complete first — security is highest priority (FR-002)
...
- **Cross-story**: User stories CANNOT run in parallel — strict priority ordering per FR-002
...
Due to FR-002 (no lower-priority fix while known higher-priority issues remain), user stories MUST be executed sequentially:
...
- Per FR-002, strict priority ordering: P1 → P2 → P3 → P4 → P5
...
- Total files to audit: ~143 backend files, ~413 frontend files, ~5 infrastructure files
- Expected output: Summary table per `specs/001-bug-basher/contracts/process-contracts.md`
