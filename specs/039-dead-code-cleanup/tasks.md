...
- [x] T012 [P] [US2] Add legacy format encounter logging (`logger.info("legacy_format_encountered: format_type=%s context=%s stage_id=%s", ...)`) at normalization points in backend/src/services/pipelines/service.py — context embedded in message string (not `extra={}`) because `StructuredJsonFormatter` only emits whitelisted extra keys
