...
The CI pipeline must enforce documentation formatting standards and validate links. This contract defines the required CI configuration and validates the existing setup against 030-doc-sweep requirements.
...
| SC-007: 100% broken links caught | markdown-link-check runs on every push/PR to main | ✅ Compliant |

## Changes Required

**None.** The existing CI configuration, markdownlint config, link-check config, and pre-commit hooks are fully compliant with FR-014–FR-016 and SC-006–SC-007. No modifications needed.
