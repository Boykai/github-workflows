...
def get_github_service(request: Request) -> GitHubProjectsService:
    ...
    if svc is None:
        raise RuntimeError(
            "github_service not set on app.state — ensure lifespan initializes it"
        )
    return svc
...
def get_connection_manager(request: Request) -> ConnectionManager:
    ...
    if mgr is None:
        raise RuntimeError(
            "connection_manager not set on app.state — ensure lifespan initializes it"
        )
    return mgr
...
def get_database(request: Request) -> aiosqlite.Connection:
    ...
    if db is None:
        raise RuntimeError(
            "db not set on app.state — ensure lifespan initializes it"
        )
    return db
