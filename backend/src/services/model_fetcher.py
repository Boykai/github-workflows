...
from src.services.completion_providers import copilot_client_pool
...
class GitHubCopilotModelFetcher(ModelFetchProvider):
    ...
    models via the shared ``CopilotClientPool``.
    """
    ...
    async def fetch_models(self, token: str | None = None) -> list[ModelOption]:
        ...
        client = await copilot_client_pool.get_or_create(token)
