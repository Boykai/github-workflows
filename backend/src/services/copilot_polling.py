...
async def check_backlog_issues(
    ...
    try:
        ...
        if not config:
            logger.debug(
                "No workflow config for project %s, skipping backlog check", project_id
            )
            return results
        ...
        logger.debug(
            "Found %d issues in '%s' status", len(backlog_tasks), config.status_backlog
        )
        ...
        for task in backlog_tasks:
            ...
            if current_agent:
                completed = (
                    await github_projects_service.check_agent_completion_comment(
                        access_token=access_token,
                        owner=task_owner,
                        repo=task_repo,
                        issue_number=task.issue_number,
...
async def check_ready_issues(
    ...
    try:
        ...
        if not config:
            logger.debug(
                "No workflow config for project %s, skipping ready check", project_id
            )
            return results
        ...
        logger.debug(
            "Found %d issues in '%s' status", len(ready_tasks), config.status_ready
        )
...
async def check_in_progress_issues(
    ...
    try:
        ...
        config = get_workflow_config(project_id)
        in_progress_label = (
            config.status_in_progress.lower() if config else "in progress"
        )
...
async def check_in_review_issues_for_copilot_review(
    ...
    try:
        ...
        in_review_tasks = [
            ...
            for task in tasks
            if task.status
            and task.status.lower() == "in review"
            and task.issue_number is not None
        ]
...
async def process_in_progress_issue(
    ...
    try:
        ...
        if (
            ...
            and pipeline.status == in_progress_status
        ):
            current_agent = pipeline.current_agent
            logger.debug(
            ...
            if main_branch_info and current_agent:
                ...
                if success:
                    ...
                    if main_pr_details and main_pr_details.get("id"):
                        review_requested = (
                            await github_projects_service.request_copilot_review(
                                access_token=access_token,
                    ...
                    logger.error(
                        "Failed to update issue #%d status to 'In Review'", issue_number
                    )
                    return {
                        "status": "error",
                        "issue_number": issue_number,
...
async def poll_for_copilot_completion(
    ...
    while _polling_state.is_running:
        try:
            ...
            all_tasks = await github_projects_service.get_project_items(
                access_token, project_id
            )
...
def get_polling_status() -> dict[str, Any]:
    ...
    return {
        ...
        "last_poll_time": (
            _polling_state.last_poll_time.isoformat()
            if _polling_state.last_poll_time
            else None
        ),
