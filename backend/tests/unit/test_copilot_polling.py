...
class TestRecoverStalledIssues:
    ...
    @pytest.mark.asyncio
    @patch("src.services.blocking_queue.store")
    @patch("src.services.copilot_polling.github_projects_service")
    @patch("src.services.copilot_polling.get_workflow_config", new_callable=AsyncMock)
    async def test_skips_pending_blocking_queue_entry(
        self, mock_config, mock_service, mock_bq_store, mock_backlog_task
    ):
        """Recovery must not assign agents to issues that are pending in the
        blocking queue — the queue will activate them at the right time."""
        ...
        pending_entry = BlockingQueueEntry(
            id=99,
            repo_key="owner/repo",
            issue_number=100,
            project_id="PVT_1",
            ...
            parent_branch=None,
            blocking_source_issue=None,
            activated_at=None,
            completed_at=None,
            created_at="2026-03-10T00:00:00Z",
        )
        mock_bq_store.get_by_issue = AsyncMock(return_value=pending_entry)
        ...
        assert results == []
        # Must not fetch the issue body or assign any agent
        mock_service.get_issue_with_comments.assert_not_called()
    ...
    @patch("src.services.copilot_polling.github_projects_service")
    @patch("src.services.copilot_polling.get_workflow_config", new_callable=AsyncMock)
    async def test_active_blocking_queue_entry_proceeds_normally(
        self,
        mock_config,
        mock_service,
        mock_get_branch,
        ...
        """An issue that IS active in the blocking queue should still go
        through normal recovery stall detection."""
        from src.models.blocking import BlockingQueueEntry, BlockingQueueStatus
        ...
        active_entry = BlockingQueueEntry(
            id=99,
            repo_key="owner/repo",
            issue_number=100,
            project_id="PVT_1",
        ...
        mock_bq_store.get_by_issue = AsyncMock(return_value=active_entry)
        mock_service.get_issue_with_comments = AsyncMock(return_value={"body": self.TRACKING_BODY})
        mock_service.is_copilot_assigned_to_issue = AsyncMock(return_value=True)
        mock_service.get_linked_pull_requests = AsyncMock(return_value=[])
        mock_get_branch.return_value = None
        mock_get_orch.return_value = MagicMock(assign_agent_for_status=AsyncMock(return_value=True))

        # Should not be skipped — issue body IS fetched for stall check
        await recover_stalled_issues(
            access_token="token",
            project_id="PVT_1",
            owner="owner",
