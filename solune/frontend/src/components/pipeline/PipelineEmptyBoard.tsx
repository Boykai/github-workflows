/**
 * PipelineEmptyBoard — empty state for the pipeline board.
 * Displays a decorative create-pipeline button with constellation theming.
 */

import { Tooltip } from '@/components/ui/tooltip';
import { ThemedAgentIcon } from '@/components/common/ThemedAgentIcon';

interface PipelineEmptyBoardProps {
  onCreatePipeline: () => void;
}

export function PipelineEmptyBoard({ onCreatePipeline }: PipelineEmptyBoardProps) {
  return (
    <div className="celestial-panel flex flex-col items-center justify-center gap-3 rounded-[1.2rem] border border-dashed border-border/60 bg-background/24 p-8 text-center">
      ...
      <p className="max-w-md text-xs text-muted-foreground">
        Build custom agent workflows by creating a pipeline with stages and agents. Click the
        constellation to get started.
      </p>
    </div>
  );
}
