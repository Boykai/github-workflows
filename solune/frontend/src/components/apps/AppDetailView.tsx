...
export function AppDetailView({ appName, onBack }: AppDetailViewProps) {
  ...
  const handleStart = () => {
    startMutation.mutate(appName, {
      ...
      onError: (err) => showSuccess(getErrorMessage(err, `Could not start app "${app.display_name}".`)),
    });
  };
  ...
  const handleStop = async () => {
    ...
    if (confirmed) {
      stopMutation.mutate(appName, {
        ...
        onError: (err) => showSuccess(getErrorMessage(err, `Could not stop app "${app.display_name}".`)),
      });
    }
  };
  ...
  const handleDelete = async () => {
    ...
    if (confirmed) {
      deleteMutation.mutate(appName, {
        onSuccess: () => onBack(),
        onError: (err) => showSuccess(getErrorMessage(err, `Could not delete app "${app.display_name}".`)),
      });
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Info Grid */}
      <dl className="grid grid-cols-2 gap-4 rounded-lg border border-zinc-200 p-4 dark:border-zinc-700 dark:bg-zinc-900 sm:grid-cols-4">
        <div>
