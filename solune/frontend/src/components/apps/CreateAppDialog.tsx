 ...
 * Supports same-repo, new-repo, and external-repo creation flows
 * with AI enhancement toggle and advanced options.
 */
...
export interface CreateAppDialogProps {
  ...
  onSubmit: (payload: AppCreate) => void;
  onClose: () => void;
  onClearError: () => void;
}
  ...
  const handleCreate = useCallback(
    (e: React.FormEvent<HTMLFormElement>) => {
      ...
      if (repoType === 'new-repo') {
        if (!repoOwner) {
          setValidationError('Repository owner is required for new repository.');
          return;
        }
        ...
        if ((trimmedAzureId && !azureSecretPresent) || (!trimmedAzureId && azureSecretPresent)) {
          setValidationError('Azure Client ID and Client Secret must both be provided or both omitted.');
          return;
        }
        ...
        if (!url) {
          setValidationError('External repository URL is required.');
          return;
        }
        ...
        if (!payload.branch) {
          setValidationError('Target branch could not be derived.');
          return;
        }
      } else {
        const branchOverride = String(formData.get('branch') ?? '').trim();
        payload.branch = branchOverride || derivedBranch;
        if (!payload.branch) {
          setValidationError('Target branch could not be derived. Please provide a display name.');
          return;
        }
      }
