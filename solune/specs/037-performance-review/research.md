...
**Rationale**: TanStack React Query v5 (used at v5.90.0) provides several invalidation strategies:
...
- `ProjectsPage` uses `useMemo` for `transformedBoardData`, `pipelineGridStyle`, `assignedPipeline`, `assignedStageMap`, and `effectiveRateLimitInfo`

Remaining opportunities:
- Verify that callback props passed to memoized components are stable (wrapped in `useCallback`) to prevent memo invalidation
- Profile whether `transformedBoardData` computation is actually expensive or just protective
- Check if the mapping of columns to `BoardColumn` components creates new arrays/objects on each render that defeat `memo()`

**Alternatives considered**: Introducing `useDeferredValue` or `useTransition` was considered but deferred as these are React 19 features that add complexity; standard memoization is the safer first-pass approach.
...
- `AddAgentPopover.tsx` uses passive scroll listeners and repositions via `requestAnimationFrame()`
