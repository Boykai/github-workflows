...
State is persisted to SQLite via ``pipeline_state_store`` for durability
across container restarts.  A BoundedDict L1 cache provides fast
synchronous reads; all mutations go through the store's public API.
"""
...
from src.services.pipeline_state_store import (
    clear_trigger_inflight_l1 as _clear_trigger_inflight_l1,
)
from src.services.pipeline_state_store import (
...
from src.services.pipeline_state_store import (
    get_all_pipeline_states as _store_get_all_pipeline_states,
)
from src.services.pipeline_state_store import (
    get_main_branch as _store_get_main_branch,
)
from src.services.pipeline_state_store import (
    get_main_branch_field as _get_main_branch_field,
)
from src.services.pipeline_state_store import (
    get_pipeline_state as _store_get_pipeline_state,
)
from src.services.pipeline_state_store import (
    get_queued_pipelines_for_project as get_queued_pipelines_for_project,
)
from src.services.pipeline_state_store import (
    get_sub_issue_map as _store_get_sub_issue_map,
)
from src.services.pipeline_state_store import (
    get_trigger_inflight_l1 as _get_trigger_inflight_l1,
)
from src.services.pipeline_state_store import (
    has_main_branch as _has_main_branch,
)
from src.services.pipeline_state_store import (
    merge_sub_issue_map_l1 as _merge_sub_issue_map_l1,
)
from src.services.pipeline_state_store import (
    persist_pipeline_state_to_db as _persist_pipeline_state_to_db,
)
from src.services.pipeline_state_store import (
    remove_main_branch_l1 as _remove_main_branch_l1,
)
from src.services.pipeline_state_store import (
    remove_pipeline_state_l1 as _remove_pipeline_state_l1,
)
from src.services.pipeline_state_store import (
    remove_sub_issue_map_l1 as _remove_sub_issue_map_l1,
)
from src.services.pipeline_state_store import (
    remove_trigger_inflight_l1 as _remove_trigger_inflight_l1,
)
from src.services.pipeline_state_store import (
    set_main_branch as _set_main_branch_async,
)
from src.services.pipeline_state_store import (
    set_main_branch_l1 as _set_main_branch_l1,
)
from src.services.pipeline_state_store import (
    set_pipeline_state_l1 as _set_pipeline_state_l1,
)
from src.services.pipeline_state_store import (
...
from src.services.pipeline_state_store import (
    set_trigger_inflight_l1 as _set_trigger_inflight_l1,
)
from src.services.pipeline_state_store import (
    update_main_branch_sha_l1 as _update_main_branch_sha_l1,
)
...
def should_skip_agent_trigger(
    ...
    inflight_started = _get_trigger_inflight_l1(key)
    ...
    _set_trigger_inflight_l1(key, now)
...
def release_agent_trigger(issue_number: int, status: str, agent_name: str) -> None:
    ...
    key = _agent_trigger_key(issue_number, status, agent_name)
    _remove_trigger_inflight_l1(key)
...
def get_pipeline_state(issue_number: int) -> PipelineState | None:
    ...
    return _store_get_pipeline_state(issue_number)


def get_all_pipeline_states() -> dict[int, PipelineState]:
    """Get all pipeline states."""
    return _store_get_all_pipeline_states()
...
def set_pipeline_state(issue_number: int, state: PipelineState) -> None:
    ...
    _set_pipeline_state_l1(issue_number, state)
...
def remove_pipeline_state(issue_number: int) -> None:
    ...
    _remove_pipeline_state_l1(issue_number)
...
def get_issue_main_branch(issue_number: int) -> MainBranchInfo | None:
    ...
    """
    return _store_get_main_branch(issue_number)
...
def get_issue_sub_issues(issue_number: int) -> dict[str, dict]:
    ...
    """
    return _store_get_sub_issue_map(issue_number)
...
def set_issue_sub_issues(issue_number: int, mappings: dict[str, dict]) -> None:
    ...
    """
    _merge_sub_issue_map_l1(issue_number, mappings)
...
def set_issue_main_branch(
    ...
    """
    if _has_main_branch(issue_number):
        logger.debug(
            "Issue #%d already has main branch set to '%s', not overwriting",
            issue_number,
            _get_main_branch_field(issue_number, "branch"),
        )
        return
    _set_main_branch_l1(issue_number, {
        "branch": branch,
        "pr_number": pr_number,
        "head_sha": head_sha,
    })
...
def clear_issue_main_branch(issue_number: int) -> None:
    ...
    _remove_main_branch_l1(issue_number)
...
def clear_issue_sub_issues(issue_number: int) -> None:
    ...
    """
    _remove_sub_issue_map_l1(issue_number)
...
def update_issue_main_branch_sha(issue_number: int, head_sha: str) -> None:
    ...
    """
    if not _has_main_branch(issue_number):
        logger.warning(
            "Cannot update HEAD SHA for issue #%d — no main branch set",
            issue_number,
        )
        return
    old_sha = _update_main_branch_sha_l1(issue_number, head_sha)
    info = _store_get_main_branch(issue_number)
    _schedule_persist(_set_main_branch_async(issue_number, info))
    logger.info(
