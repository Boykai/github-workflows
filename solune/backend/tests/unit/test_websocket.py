...
class TestDisconnectLockConsistency:
    """Regression test: disconnect must acquire the websocket lock so it
    cannot interleave with connect (bug-bash fix)."""
    ...
    @pytest.mark.asyncio
    async def test_disconnect_is_async(self, manager):
        """disconnect() must be a coroutine so it can acquire the async lock."""
        import asyncio

        ws = MagicMock()
        ws.accept = AsyncMock()
        await manager.connect(ws, "PVT_LOCK")

        # disconnect must be awaitable (async)
        result = manager.disconnect(ws)
        assert asyncio.iscoroutine(result)
        await result

        assert manager.get_connection_count("PVT_LOCK") == 0
