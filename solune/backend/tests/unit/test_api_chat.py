...
class TestSendMessageTaskGeneration:
    ...
    async def test_ai_enhance_on_uses_full_pipeline(
        ...
        # The conftest-provided ChatAgentService mock already returns
        # a task_create response with "Generated Task" title.
        resp = await client.post(
            "/api/v1/chat/messages",
        ...
        data = resp.json()
        assert data["action_data"]["proposed_title"] == "Generated Task"
        assert data["action_data"]["proposed_description"] == "Generated description"
